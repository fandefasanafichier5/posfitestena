...
export const sendReceiptByEmail = async (
  ...
  try {
    ...
    const itemsContent = receiptData.items.map((item: any) => 
      `${item.name} x${item.quantity} - ${new Intl.NumberFormat('mg-MG', {
        style: 'currency',
        currency: 'MGA',
        minimumFractionDigits: 0
      }).format(item.total).replace('MGA', 'Ar')}`
    ).join('\n');
    ...
    const templateParams = {
      ...
      subtotal: new Intl.NumberFormat('mg-MG', {
        style: 'currency',
        currency: 'MGA',
        minimumFractionDigits: 0
      }).format(receiptData.subtotal).replace('MGA', 'Ar'),
      tax_amount: new Intl.NumberFormat('mg-MG', {
        style: 'currency',
        currency: 'MGA',
        minimumFractionDigits: 0
      }).format(receiptData.taxAmount).replace('MGA', 'Ar'),
      total_amount: new Intl.NumberFormat('mg-MG', {
        style: 'currency',
        currency: 'MGA',
        minimumFractionDigits: 0
      }).format(receiptData.total).replace('MGA', 'Ar'),
      payment_method: receiptData.paymentMethod,
      payment_amount: new Intl.NumberFormat('mg-MG', {
        style: 'currency',
        currency: 'MGA',
        minimumFractionDigits: 0
      }).format(receiptData.paymentAmount).replace('MGA', 'Ar'),
      change_amount: receiptData.changeAmount > 0 ? new Intl.NumberFormat('mg-MG', {
        style: 'currency',
        currency: 'MGA',
        minimumFractionDigits: 0
      }).format(receiptData.changeAmount).replace('MGA', 'Ar') : '0 Ar'
    };
